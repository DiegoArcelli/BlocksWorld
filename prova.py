...
def predict(image, previous):
    ...
    for i in range(28):
        for j in range(28):
            if mask[i, j] != 0:
                mask[i, j] = 255
    _in = mask.reshape((784))
    _in = _in / 255
    _list = [_in]
    _list = np.array(_list)
    digit = np.argmax(model.predict(_list[:1]))
    # print(digit)
    # show(mask)
...
def find_digits(image, previous, org):
    ...
    for contour in contours:
        ...
        if validate_figure(contour, approx, image.shape[::-1], org.shape[::-1]):
            find_digits(image[y:y+h, x:x+w], image.copy(), org)
...
def process_image_file(filename):
    ...
    find_digits(image_out, None, image_out.copy())
    for i in range(6):
        if deteced[i].size > 0:
            print(i)
            show(deteced[i])
...
box = process_image_file("./images/sample0/initial.jpg")
